...
wstring CMyParticle::ObjID = TEXT("CMyParticle");

CMyParticle::CMyParticle(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CEffect_Base{ pDevice, pContext }
{
	s_PrototypeTags.insert(CMyParticle::ObjID);
}

CMyParticle::CMyParticle(const CMyParticle& rhs)
	: CEffect_Base(rhs)
{
}
...
HRESULT CMyParticle::Initialize_Prototype()
{
	...
	InstanceDesc.vScale = _float2(0.2f, 1.f);
	...
	if (FAILED(GAMEINSTANCE->Add_Prototype(LEVEL_STATIC, TEXT("Prototype_Component_VIBuffer_PointInstance_MyParticle"),
...
HRESULT CMyParticle::Initialize(void* pArg)
{
	...
	m_pVIBufferCom->Set_Style(CVIBuffer_Point_Instance::STYLE_DROP);
	EffectInit();

	return S_OK;
}

void CMyParticle::PriorityTick(_float fTimeDelta)
{
}

void CMyParticle::Tick(_float fTimeDelta)
{
...
void CMyParticle::LateTick(_float fTimeDelta)
{
	m_pGameInstance.lock()->Add_RenderObjects(CRenderer::RENDER_BLEND_NON_LIT, shared_from_this());
}

HRESULT CMyParticle::Render()
{
	_float4x4 ViewMat;
...
void CMyParticle::Push_From_Pool(void* pArg)
{
	//m_pVIBufferCom->Get_VertexPos();
	...
	//XMStoreFloat3(&m_vDir, m_pTransformCom->Get_NormLook() * m_pTransformCom->Get_Speed());
	m_pTransformCom->Set_WorldMatrix(Identity);
	vector<_float3>* desc = (vector<_float3>*)pArg;
	if(desc->size())
	{
		_float3 pos = desc->back();
		desc->pop_back();
		m_pVIBufferCom->Set_Particle_Positions(*desc);
		m_pTransformCom->Set_Position({ pos.x, pos.y, pos.z, 1.f });
	}
...
void CMyParticle::Dying()
{
	EffectEnd();
}

void CMyParticle::EffectStartCallBack()
{
}

void CMyParticle::EffectEndCallBack()
{
...
void CMyParticle::EffectSoftEnd()
{
}

void CMyParticle::EffectHardEnd()
{
}

_float3 CMyParticle::Get_Pivot()
{
	if (m_pVIBufferCom == nullptr)
...
void CMyParticle::Set_Pivot(_float3 vPivot)
{
	if (m_pVIBufferCom == nullptr)
		return;
...
void CMyParticle::Set_Speed(_float2 vSpeed)
{
	if (m_pVIBufferCom == nullptr)
		return;
...
void CMyParticle::Set_Range(_float3 vRange)
{
	if (m_pVIBufferCom == nullptr)
		return;
...
_bool CMyParticle::Get_IsRandomize()
{
	if (m_pVIBufferCom == nullptr)
		return false;
...
void CMyParticle::Set_IsRandomize(_bool bIsRandomize)
{
	if (m_pVIBufferCom == nullptr)
		return;
...
void CMyParticle::Set_Scale(_float2 vScale)
{
	if (m_pVIBufferCom == nullptr)
		return;
...
CVIBuffer_Instance::INSTANCE_STYLE CMyParticle::Get_Style()
{
	if (m_pVIBufferCom == nullptr)
		assert(false);
...
void CMyParticle::Set_Style(CVIBuffer_Instance::INSTANCE_STYLE eStyle)
{
	if (m_pVIBufferCom == nullptr)
		return;
...
HRESULT CMyParticle::Ready_Components()
{
	/* For.Com_Shader */
	...
	m_pVIBufferCom = static_pointer_cast<CVIBuffer_Point_Instance>(Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_VIBuffer_PointInstance_MyParticle"), TEXT("Com_VIBuffer")));
...
shared_ptr<CMyParticle> CMyParticle::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	struct MakeSharedEnabler :public CMyParticle
	{
		MakeSharedEnabler(ID3D11Device* pDevice, ID3D11DeviceContext* pContext) : CMyParticle(pDevice, pContext)
		{
		}
	};


	shared_ptr<CMyParticle> pInstance = make_shared<MakeSharedEnabler>(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CMyParticle"));
		assert(false);
	}
...
shared_ptr<CGameObject> CMyParticle::Clone(void* pArg)
{
	struct MakeSharedEnabler :public CMyParticle
	{
		MakeSharedEnabler(const CMyParticle& rhs) : CMyParticle(rhs) {}
	};

	shared_ptr<CMyParticle> pInstance = make_shared<MakeSharedEnabler>(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed to Cloned : CMyParticle"));
		assert(false);
	}

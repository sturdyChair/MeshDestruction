...
class ENGINE_DLL CModel_Cutter : public CComponent
{
public:
...
private:
	CModel_Cutter(ID3D11Device* pDevice, ID3D11DeviceContext* pContext);
	CModel_Cutter(const CModel_Cutter& rhs);
public:
	virtual ~CModel_Cutter();

public:
	virtual HRESULT Initialize_Prototype(const _tchar* pShaderFilePath);
	virtual HRESULT Initialize(void* pArg) override;

	pair<vector<_uint3>, vector<_uint3>> Cut(const _float4& Plane, vector<VTXANIMMESH>& out);

private:
	void Shader_Compute();
	void Bind_CutPlane();
	void Bind_Bones();
	void Get_NewVertices(vector<VTXANIMMESH>& out);
...
private:
	...
	//ID3D11Buffer*				m_pOutputNewEdgeBuffer = nullptr;
	//ID3D11UnorderedAccessView*	m_pOutputNewEdgeUAV = nullptr;
	...
	//ID3D11Buffer*				m_pStagingEdgeBuffer = nullptr;
	ID3D11Buffer*				m_pStagingIndiciesBuffer = nullptr;
	ID3D11Buffer*				m_pCountBuffer = nullptr;
public:
	static shared_ptr<CModel_Cutter> Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, const _tchar* pShaderFilePath);
	virtual shared_ptr<CComponent> Clone(void* pArg) override;
	void Free();


	friend class CCutter_Controller;
	friend class CMultiCutterController;
};
